...
// #pragma warning(pop)
  ...
  while (lasreadopener.active()) {
    ...
    if (opts) {
      ...
      if (parse_string && strcmp(parse_string, "xyz") && strcmp(parse_string, "xyzi") && strcmp(parse_string, "xyziRGB") &&
          strcmp(parse_string, "xyzRGB")) {
      ...
      if (ptxVLR && (ptxVLR->record_length_after_header == 272)) {
        ...
        fprintf(
            file_out, "%g %g %g %g\012", ((F64*)payload)[18], ((F64*)payload)[19], ((F64*)payload)[20],
            ((F64*)payload)[21]);  // r11 r12 r13 - rotation matrix
        ...
        fprintf(
            file_out, "%g %g %g %g\012", ((F64*)payload)[30], ((F64*)payload)[31], ((F64*)payload)[32],
            ((F64*)payload)[33]);  // tr1 tr2 tr3 - transformation
      } else {
        if (ptxVLR) {
          LASMessage(LAS_WARNING, "found VLR for PTX with wrong payload size of %d.", ptxVLR->record_length_after_header);
        } else {
      ...
      fprintf(file_out, "%c project ID GUID data 1-4:  %s\012", header_comment_sign, header->get_GUID().c_str());
      fprintf(file_out, "%c version major.minor:       %s\012", header_comment_sign, header->get_version().c_str());
      ...
      fprintf(
          file_out, "%c number of points by return %u %u %u %u %u\012", header_comment_sign, header->number_of_points_by_return[0],
          header->number_of_points_by_return[1], header->number_of_points_by_return[2], header->number_of_points_by_return[3],
          header->number_of_points_by_return[4]);
      fprintf(
          file_out, "%c scale factor x y z         %g %g %g\012", header_comment_sign, header->x_scale_factor, header->y_scale_factor,
          header->z_scale_factor);
      fprintf(file_out, "%c offset x y z               ", header_comment_sign);
      lidardouble2string(printstring, header->x_offset);
      fprintf(file_out, "%s ", printstring);
      lidardouble2string(printstring, header->y_offset);
      fprintf(file_out, "%s ", printstring);
      lidardouble2string(printstring, header->z_offset);
      fprintf(file_out, "%s\012", printstring);
      fprintf(file_out, "%c min x y z                  ", header_comment_sign);
      lidardouble2string(printstring, header->min_x, header->x_scale_factor);
      fprintf(file_out, "%s ", printstring);
      lidardouble2string(printstring, header->min_y, header->y_scale_factor);
      fprintf(file_out, "%s ", printstring);
      lidardouble2string(printstring, header->min_z, header->z_scale_factor);
      fprintf(file_out, "%s\012", printstring);
      fprintf(file_out, "%c max x y z                  ", header_comment_sign);
      lidardouble2string(printstring, header->max_x, header->x_scale_factor);
      fprintf(file_out, "%s ", printstring);
      lidardouble2string(printstring, header->max_y, header->y_scale_factor);
      fprintf(file_out, "%s ", printstring);
      lidardouble2string(printstring, header->max_z, header->z_scale_factor);
      fprintf(file_out, "%s\012", printstring);
      // if LAS 1.4
      if (header->version_minor >= 4) {
        fprintf(file_out, "%c extended number of point records    %lld\012", header_comment_sign, header->extended_number_of_point_records);
        fprintf(
            file_out, "%c extended number of points by return %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld %lld\012",
            header_comment_sign, header->extended_number_of_points_by_return[0], header->extended_number_of_points_by_return[1],
            header->extended_number_of_points_by_return[2], header->extended_number_of_points_by_return[3],
            header->extended_number_of_points_by_return[4], header->extended_number_of_points_by_return[5],
            header->extended_number_of_points_by_return[6], header->extended_number_of_points_by_return[7],
            header->extended_number_of_points_by_return[8], header->extended_number_of_points_by_return[9],
            header->extended_number_of_points_by_return[10], header->extended_number_of_points_by_return[11],
            header->extended_number_of_points_by_return[12], header->extended_number_of_points_by_return[13],
            header->extended_number_of_points_by_return[14]);
      }
    }
    ...
    while (parse_string[i]) {
      switch (parse_string[i]) {
        ...
        case 'M':  // the index of the point (count starts at 1)
          break;
        case 'l':  // the (extended) scanner channe<l>
          if (lasreader->point.extended_point_type == 0) LASMessage(LAS_WARNING, "requested 'l' but points are not of extended type");
          break;
        case 'o':  // the (extended) <o>verlap flag
          if (lasreader->point.extended_point_type == 0) LASMessage(LAS_WARNING, "requested 'o' but points are not of extended type");
          break;
        case '#':  // diff of gps-time to prev point
        case 't':  // the gps-time
          if (lasreader->point.have_gps_time == false) LASMessage(LAS_WARNING, "requested 't' but points do not have gps time");
          break;
        case '$':  // the R difference to the last point
        case '&':  // the byte-wise R difference to the last point
        case 'R':  // the red channel of the RGB field
          if (lasreader->point.have_rgb == false) LASMessage(LAS_WARNING, "requested 'R' but points do not have RGB");
          break;
        case '%':  // the G difference to the last point
        case '*':  // the byte-wise G difference to the last point
        case 'G':  // the green channel of the RGB field
          if (lasreader->point.have_rgb == false) LASMessage(LAS_WARNING, "requested 'G' but points do not have RGB");
          break;
        case '^':  // the B difference to the last point
        case '+':  // the byte-wise B difference to the last point
        case 'B':  // the blue channel of the RGB field
          if (lasreader->point.have_rgb == false) LASMessage(LAS_WARNING, "requested 'B' but points do not have RGB");
          break;
        case HSL:
        case HSV:
        case HSL255:
        case HSV255:
          if (lasreader->point.have_rgb == false) LASMessage(LAS_WARNING, "requested 'HSV' or 'HSL' but points do not have RGB");
          break;
        case 'I':  // the near infrared channel of the RGBI field
          if (lasreader->point.have_nir == false) LASMessage(LAS_WARNING, "requested 'I' but points do not have NIR");
          break;
        case 'w':  // the wavepacket index
          if (lasreader->point.have_wavepacket == false) LASMessage(LAS_WARNING, "requested 'w' but points do not have wavepacket");
          break;
        case 'W':  // all wavepacket attributes
          if (lasreader->point.have_wavepacket == false) LASMessage(LAS_WARNING, "requested 'W' but points do not have wavepacket");
          break;
    ...
    if (coldesc) {
      ...
      while (true) {
        switch (parse_string[i]) {
          case 'x':
            fprintf(file_out, "x");
            break;  // the z coordinate
          case 'y':
            fprintf(file_out, "y");
            break;  // the y coordinate
          case 'z':
            fprintf(file_out, "z");
            break;  // the z coordinate
          case 'X':
            fprintf(file_out, "X");
            break;  // the unscaled and unoffset integer X coordinate
          case 'Y':
            fprintf(file_out, "Y");
            break;  // the unscaled and unoffset integer Y coordinate
          case 'Z':
            fprintf(file_out, "Z");
            break;  // the unscaled and unoffset integer Z coordinate
          case 't':
            fprintf(file_out, "gps_time");
            break;  // the gps-time
          case 'i':
            fprintf(file_out, "intensity");
            break;  // the intensity
          case 'a':
            fprintf(file_out, "scan_angle");
            break;  // the scan angle
          case 'r':
            fprintf(file_out, "return_number");
            break;  // the number of the return
          case 'c':
            fprintf(file_out, "classification");
            break;  // the classification
          case 'u':
            fprintf(file_out, "user_data");
            break;  // the user data
          case 'n':
            fprintf(file_out, "number_of_returns");
            break;  // the number of returns of given pulse
          case 'p':
            fprintf(file_out, "point_source_id");
            break;  // the point source ID
          case 'e':
            fprintf(file_out, "edge_of_flight_line");
            break;  // the edge of flight line flag
          case 'd':
            fprintf(file_out, "scan_direction_flag");
            break;  // the direction of scan flag
          case 'h':
            fprintf(file_out, "withheld_flag");
            break;  // the withheld flag
          case 'k':
            fprintf(file_out, "keypoint_flag");
            break;  // the keypoint flag
          case 'g':
            fprintf(file_out, "synthetic_flag");
            break;  // the synthetic flag
          case 'o':
            fprintf(file_out, "overlap_flag");
            break;  // the overlap flag
          case 'l':
            fprintf(file_out, "scanner_channel");
            break;  // the scanner channel
          case 'R':
            fprintf(file_out, "R");
            break;  // the red channel of the RGB field
          case 'G':
            fprintf(file_out, "G");
            break;  // the green channel of the RGB field
          case 'B':
            fprintf(file_out, "B");
            break;  // the blue channel of the RGB field
          case 'm':
            fprintf(file_out, "0-index");
            break;  // the index of the point (count starts at 0)
          case 'M':
            fprintf(file_out, "1-index");
            break;  // the index of the point (count starts at 1)
          case '_':
            fprintf(file_out, "raw_int_X_diff");
            break;  // the raw integer X difference to the last point
          case '!':
            fprintf(file_out, "raw_int_Y_diff");
            break;  // the raw integer Y difference to the last point
          case '@':
            fprintf(file_out, "raw_int_Z_diff");
            break;  // the raw integer Z difference to the last point
          case '#':
            fprintf(file_out, "gpstime_diff");
            break;  // the gps-time difference to the last point
          case '$':
            fprintf(file_out, "R_diff");
            break;  // the R difference to the last point
          case '%':
            fprintf(file_out, "G_diff");
            break;  // the G difference to the last point
          case '^':
            fprintf(file_out, "B_diff");
            break;  // the B difference to the last point
          case '&':
            fprintf(file_out, "bitwise_R_diff");
            break;  // the byte-wise R difference to the last point
          case '+':
            fprintf(file_out, "bitwise_B_diff");
            break;  // the byte-wise B difference to the last point
          case 'w':
            fprintf(file_out, "wavepacket_descriptor_index");
            break;  // the wavepacket descriptor index
          case 'W':
            fprintf(
                file_out, "wavepacket_descriptor_index%cwavepacket_offset%cwavepacket_size%cwavepacket_location%cXt%cYt%cZt", separator_sign,
                separator_sign, separator_sign, separator_sign, separator_sign, separator_sign);
            break;  // all wavepacket attributes
          case 'V':
            break;
          case 'E':
            break;
          case HSV255:
            fprintf(file_out, "HSV_H%cHSV_S%cHSV_V", separator_sign, separator_sign);
            break;  // the HSV representation of RGB in [0,360|100]
          case HSL255:
            fprintf(file_out, "HSL_H%cHSL_S%cHSL_L", separator_sign, separator_sign);
            break;  // the HSL representation of RGB in [0,360|100]
          case HSV:
            fprintf(file_out, "HSV_h%cHSV_s%cHSV_v", separator_sign, separator_sign);
            break;  // the HSV representation of RGB in [0, 1]
          case HSL:
            fprintf(file_out, "HSL_h%cHSL_s%cHSL_l", separator_sign, separator_sign);
            break;  // the HSL representation of RGB in [0, 1]
          default:
            break;  // must handle extra bytes
        }
    ...
    while (lasreader->read_point()) {
      ...
      while (true) {
        switch (parse_string[i]) {
          case 'x':  // the x coordinate
            lidardouble2string(printstring, lasreader->point.get_x(), lasreader->header.x_scale_factor);
            fprintf(file_out, "%s", printstring);
            break;
          case 'y':  // the y coordinate
            lidardouble2string(printstring, lasreader->point.get_y(), lasreader->header.y_scale_factor);
            fprintf(file_out, "%s", printstring);
            break;
          case 'z':  // the z coordinate
            lidardouble2string(printstring, lasreader->point.get_z(), lasreader->header.z_scale_factor);
            fprintf(file_out, "%s", printstring);
            break;
          case 'X':  // the unscaled raw integer X coordinate
          ...
          case '#':  // the gps-time difference to the last point
            lidardouble2string(printstring, lasreader->point.gps_time - last_GPSTIME);
            fprintf(file_out, "%s", printstring);
            break;
          case '$':  // the R difference to the last point
          ...
          case '&':  // the byte-wise R difference to the last point
            fprintf(
                file_out, "%d%c%d", (lasreader->point.rgb[0] >> 8) - (last_RGB[0] >> 8), separator_sign,
                (lasreader->point.rgb[0] & 255) - (last_RGB[0] & 255));
            break;
          case '*':  // the byte-wise G difference to the last point
            fprintf(
                file_out, "%d%c%d", (lasreader->point.rgb[1] >> 8) - (last_RGB[1] >> 8), separator_sign,
                (lasreader->point.rgb[1] & 255) - (last_RGB[1] & 255));
            break;
          case '+':  // the byte-wise B difference to the last point
            fprintf(
                file_out, "%d%c%d", (lasreader->point.rgb[2] >> 8) - (last_RGB[2] >> 8), separator_sign,
                (lasreader->point.rgb[2] & 255) - (last_RGB[2] & 255));
            break;
          case 'w':  // the wavepacket index
          ...
          case 'W':  // all wavepacket attributes
            fprintf(
                file_out, "%d%c%d%c%d%c%g%c%g%c%g%c%g", lasreader->point.wavepacket.getIndex(), separator_sign,
                (U32)lasreader->point.wavepacket.getOffset(), separator_sign, lasreader->point.wavepacket.getSize(), separator_sign,
                lasreader->point.wavepacket.getLocation(), separator_sign, lasreader->point.wavepacket.getXt(), separator_sign,
                lasreader->point.wavepacket.getYt(), separator_sign, lasreader->point.wavepacket.getZt());
            break;
          case 'V':  // the waVeform
